...
| 2026-05-12 | Ajustes visuais pré-aprovação Fase 2: contadores numéricos de fila ("Carros: N", "Ped L: N", "Ped O: N") em texto grande sempre visíveis no cruzamento; indicadores piscantes de chegadas (+N, amarelo) e escoamentos (−N, ciano) por ~5 frames a cada tick. **Pendência Fase 3:** quando o gerador de cenários estiver integrado, o HUD do demo deve exibir faixa horária, família e tipo de dia do cenário em execução. |
| 2026-05-12 | Fase 2 aprovada pelo usuário. Visualização Pygame + benchmark de tempo fixo (45s carros / 25s pedestres) funcionais. HUD com contadores numéricos, indicadores +N/-N de chegada e escoamento, controles de velocidade 1x/5x/10x/50x, pausa, reset. Pendência registrada para Fase 3: HUD deve mostrar faixa horária, família e tipo de dia quando o gerador de cenários for integrado. |
| 2026-05-14 | Fase 3 concluída. Gerador de cenários implementado com duas camadas (perfil-base × família × variabilidade × Poisson). 82 cenários gerados (50 treino + 32 avaliação). Calibração realista com taxas baseadas em referências de SP (capacidade 3 carros/tick por drenagem fracionada com residual, 4 pedestres/tick por lado). Métricas refatoradas para registry configurável com 13 métricas. Benchmark de tempo fixo (45s/25s) executado: ~108k violações de teto, com pico_veic e pico_ped sendo cenários estruturalmente difíceis para o controlador fixo — esperado como sinal forte para o RL. Validações visuais A/B/C produzidas e revisadas. Cobertura de testes: 34/34 passando. |
| 2026-05-14 | Fase 3.5 aprovada. Checkpoint pré-ML concluído com 116/116 testes passando, relatório consolidado em results/fase3_5_checkpoint.md. Próximo passo: handoff do projeto para Letícia (membro do grupo) que conduzirá as Fases 4 (ambiente Gymnasium) e 5 (treino PPO). |
